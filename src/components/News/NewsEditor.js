...
function NewsAdd({
	...
	function editorNews (){
		validateFields((err, fieldsValue) => {
			if(!err){
				fieldsValue.newsflashBottomImg = imageUrl==''?null:imageUrl
				const data = {
					newsflashId:NewsFlashItem.newsflashId,
					updateUser:merId,
					ifPush:ifPushValue,
					...fieldsValue
				}
				dispatch({
					type:"news/updateNewsFlash",
					payload:{
						...data
					}
				})
		...
		<Form>
			<FormItem label="快讯标题" {...formItemLayout}>
				{getFieldDecorator('newsflashTitle', {
					initialValue:NewsFlashItem&&NewsFlashItem.newsflashTitle,
					rules: [{
						type: 'string',
			...
			<FormItem label="快讯内容" {...formItemLayout}>
				{getFieldDecorator('newsflashText', {
					initialValue:NewsFlashItem&&NewsFlashItem.newsflashText,
					rules: [{
						type: 'string',
			...
			<FormItem label="重要级别" {...formItemLayout} extra="标记为重要的快讯将在前端以红色字体展示">
				{getFieldDecorator('level', {
					initialValue:NewsFlashItem&&NewsFlashItem.level+'',
					rules: [{
			...
			<FormItem label="快讯分类" {...formItemLayout}>
				{getFieldDecorator('newsflashType', {
					initialValue:NewsFlashItem&&NewsFlashItem.newsflashType+'',
					rules: [{
			...
			<FormItem label="原文链接" {...formItemLayout}>
				{getFieldDecorator('newsflashLink', {
					initialValue:NewsFlashItem&&(NewsFlashItem.newsflashLink!=null?NewsFlashItem.newsflashLink+'':''),
					rules: [{
						required: false,
					}
					],
			...
			<FormItem label="是否推送" {...formItemLayout}>
					...
					<RadioGroup onChange={ifPushChange} defaultValue={ifPushValue&&ifPushValue} value={ifPushValue&&ifPushValue} disabled={(NewsFlashItem && NewsFlashItem.ifPush == 0) ? false : true}>
			...
			<FormItem label="显示状态" {...formItemLayout}>
				{getFieldDecorator('displayStatus', {
					initialValue:NewsFlashItem&&NewsFlashItem.displayStatus+'',
					rules: [{
						type: 'string',
			...
			<FormItem label="&emsp;" {...formItemLayout} colon={false}>
			    <Button type="primary" size="large" style={{ paddingLeft: 20, paddingRight: 20 }} onClick={()=>editorNews()} loading={confirmLoading}>保存</Button>
